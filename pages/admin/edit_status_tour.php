...
$sql_update_status = "UPDATE wpzu_trav_tour_bookings SET status = $status WHERE id = $booking_id";

if (mysqli_query($con, $sql_update_status)) {
    if ($status == 2) {
        $sql_select_booking = "SELECT email, first_name, last_name, booking_no FROM wpzu_trav_tour_bookings WHERE id = $booking_id";
        ...
        $subject = "Konfirmasi Booking Tour - Sunrise Indonesia ";
